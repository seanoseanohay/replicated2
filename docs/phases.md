...
Goal: Capture logs from every app component for offline troubleshooting
Deliverables: troubleshoot.sh support bundle spec with dedicated logs collectors for all components, embedded in Helm chart as Secret
Success: `kubectl support-bundle` generates a tar.gz containing non-empty logs for backend, worker, beat, frontend, SDK, minio, postgresql, and redis
Built: chart/templates/support-bundle.yaml — Kubernetes Secret with troubleshoot.sh/kind: support-bundle label containing support-bundle-spec; 8 dedicated logs collectors targeting each component by label selector (backend, worker, beat, frontend, sdk, minio, postgresql, redis) with `maxLines: 10000` and `maxAge: 720h` limits; `clusterInfo` and `clusterResources` collectors for context; verified with `kubectl support-bundle` plugin — generated archive contains non-empty logs for all 8 components (backend 1.3MB, frontend 258KB, sdk 202KB, worker 22KB, postgresql 9KB, beat 7KB, redis 1.4KB, minio 504B)
